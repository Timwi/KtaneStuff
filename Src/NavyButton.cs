...
namespace KtaneStuff
{
    static class NavyButton
    {
        ...
        {
            var allLatinSquares = FindSolutions(new int?[_sz * _sz], null, null).ToList();
        ...
        {
            Stencil_FindPossibleWords(FindSolutions(new int?[_sz * _sz], null, null).ToArray());
        }
        ...
        {
            var allLatinSquares = FindSolutions(new int?[_sz * _sz], null, null).ToArray();
        ...
        private static readonly (int dx, int dy)[][] _stencils = Ut.NewArray<(int dx, int dy)[]>(
            new[] { (1, -1), (1, 0), (-1, 1) },
            new[] { (0, -1), (1, 0), (0, 1) },
            new[] { (0, -1), (1, 0), (-1, 1) },
            new[] { (-1, -1), (-1, 0), (0, 1) },
        ...
        public static void GatherData()
        {
            ...
            Enumerable.Range(0, latinSquares.Length).ParallelForEach(Environment.ProcessorCount, lsqIx =>
            {
                ...
                IEnumerable<int[]> findStencilCombinations(int[] sofar, int ix)
                {
                    ...
                    for (var stIx = 0; stIx < _stencils.Length; stIx++)
                    {
                        if (_stencils[stIx].Any(tup => lsq[threes[ix].AddWrap(tup.dx, tup.dy).Index] == 3))
                            continue;
                        if (_stencils[stIx].All(tup => lsq[threes[ix].AddWrap(tup.dx, tup.dy).Index] == 0))
                            continue;
                        sofar[ix] = stIx;
                ...
                {
                    var word = comb.Select((stIx, ix) => (char) (_stencils[stIx].Aggregate(0, (p, n) => p * 3 + lsq[threes[ix].AddWrap(n.dx, n.dy).Index]) + 'A' - 1)).JoinString();
                    var addedExtraStencil = _stencils.IndexOf(extraSt => "1234,2340,3401,4012".Split(',')
                            .Select(str => str.Select(ch => ch - '0').Select(ix => ix == 4 ? extraSt : _stencils[comb[ix]]))
            ...
            Utils.ReplaceInFile(@"D:\c\KTANE\BunchOfButtons\Lib\NavyButtonData.cs", "/*stencils-start*/", "/*stencils-end*/",
               _stencils.Select(ta => $"new[] {{ {ta.Select(tup => $"({tup.dx}, {tup.dy})").JoinString(", ")} }}").JoinString(", "));
        }
        ...
        public static void GenerateManualGraphics()
        {
            ...
            Utils.ReplaceInFile(@"D:\c\KTANE\Public\HTML\The Navy Button.html", "<!--%%-->", "<!--%%%-->",
                Enumerable.Range(0, 7).Select(row => $@"<tr><th><img src='img/The Navy Button/{shapeNames[row]}.png' /></th>{Enumerable.Range(0, 3).Select(col =>
                    $"<td><svg viewBox='0 0 3 3' fill='none' stroke='black' stroke-width='.05'><path d='M1 1h1v1H1z' />{_stencils[col + 3 * row].Select(tup => $"<circle cx='{tup.dx + 1.5}' cy='{tup.dy + 1.5}' r='.4' />").JoinString()}</svg></td>").JoinString()}</tr>").JoinString());
        }
    }
}