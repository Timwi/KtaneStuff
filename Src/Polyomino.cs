...
public class Polyomino : IEquatable<Polyomino>
{
    ...
    private Polyomino() { }
    public Polyomino(int w, int h, bool[] arr)
    {
        if (arr == null || arr.Length != w * h)
            throw new ArgumentException("Polyomino array length must match width times height.", nameof(arr));
        _w = w;
        _h = h;
        _arr = arr;
    }
    ...
    public Polyomino RotateClockwise() => new(_h, _w, Ut.NewArray(_h * _w, ix => _arr[(ix / _h) + _w * (_h - 1 - (ix % _h))]));
    public Polyomino Reflect() => new(_w, _h, Ut.NewArray(_w * _h, ix => _arr[_w - 1 - (ix % _w) + _w * (ix / _w)]));
