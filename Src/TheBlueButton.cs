...
namespace KtaneStuff
{
    ...
    static class TheBlueButton
    {
        ...
        private static IEnumerable<VertexInfo[]> ColorBlob()
        {
            ...
            var data = new (double width, double depth, double bevel, Normal? normal)[] { (.4, .3, .2, null), (.7, .2, .3, n), (1, 0, .4, Normal.Mine) };
            ...
            return CreateMesh(false, true, data.Select(rect2).ToArray()).Concat(rect2(data[0]).Select(m => m.Location.WithNormal(0, 1, 0)).ToArray());
        }
