...
namespace GridLock {{
    static class Data {{
        ...
        {
            Pt transmogrify(Pt v)
            {
                var x = -v.X * 0.135 + .66;
                var z = v.Z * 0.135 - .66;
                return pt(x, v.Y + .15, z);
            }

            const double lx = -1.25;
            const double ty = -1.25;
            const double rx = 8;
            const double by = 1.25;

            var a = pt(lx + .4 - .1 * tan(22.5), 0, ty - .1);
            var b = pt(rx - .4 + .1 * tan(22.5), 0, ty - .1);
            var e = pt(lx - .1, 0, ty + .4 - .1 * tan(22.5));
            var f = pt(rx + .1, 0, ty + .4 - .1 * tan(22.5));
            var k = pt(lx - .1, 0, by - .4 + .1 * tan(22.5));
            var l = pt(rx + .1, 0, by - .4 + .1 * tan(22.5));
            var o = pt(lx + .4 - .1 * tan(22.5), 0, by + .1);
            var p = pt(rx - .4 + .1 * tan(22.5), 0, by + .1);

            yield return new[] { a, b, f, l, p, o, k, e }.Select(transmogrify).Reverse().ToArray();
        }
