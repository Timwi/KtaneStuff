...
namespace KtaneStuff
{
    public static class ColorGenerator
    {
        public static void GenerateCheatSheet()
        {
            ...
            {
                string ch(int n) => Enumerable.Range(0, 10).Where(v => v == n).Select(v => (char) ('0' + v)).Concat(Enumerable.Range(0, 26).Where(v => v % 16 == n).Select(v => (char) ('A' + v))).JoinString();
                Utils.ReplaceInFile(@"D:\c\KTANE\Public\HTML\Color Generator cheat sheet (Timwi).html", "<!--%%2-->", "<!--%%%2-->",
                    new TABLE { class_ = "color-generator" }._(
                        new TR(new TD { class_ = "corner" }, Enumerable.Range(0, 16).Select(i => new TH(ch(i)))),
                        Enumerable.Range(0, 16).Select(row => new TR(
                            new TH(ch(row)),
